...
arma::vec no_reweighting_sum_C(arma::mat Ymat, arma::vec Y, arma::vec Yflag, arma::vec prior_weight, bool ImputeAll){
  ...
  if(ImputeAll)  {
    ...
    for(int i = 0; i < p; ++i){
       ...
       res(i) = sum(prior_weight%Ymat.col(i));
    ...
    for(int i = 0; i < to_impute_num; ++i){
      ...
      int id = to_impute(i);
      res(id) = sum(prior_weight%Ymat.col(id));
...
Rcpp::List imputation_by_samples_without_reweighting(arma::mat data, arma::mat selected_logxx, arma::mat logxx, arma::mat zero_matrix, int n, int p, bool minbool){
  ...
  for(int j = 0; j < n; ++j){
    ...
    if(selected.n_elem < 3){
      ...
      imputed.col(j) = no_reweighting_sum_C(Ymat, logxx.col(j), zero_matrix.col(j), sub_prior_weight, TRUE);
  ...
  if(ImputeAll)  {
    ...
    for(int i = 0; i < p; ++i){
      ...
      if(zero_num < k_cut & zero_num != 0){
        ...
        arma::uvec Ysub_ind_zero = arma::find(Yflagmat.col(i) == 0);
        double zero_num_sub = Ysub_ind_zero.n_elem;

        for(int j = 0; j < zero_num_sub; ++j){
          int zero_id = Ysub_ind_zero(j);
          Ymat(zero_id, i) = expected;
        }
...
Rcpp::List imputation_by_samples_expectation_partial(arma::mat data, arma::mat selected_logxx, arma::mat logxx, arma::mat zero_matrix, int n, int p, bool minbool){
  ...
  for(int j = 0; j < n; ++j){
    ...
    if(selected.n_elem < 3){
      ...
      imputed.col(j) = replacing_by_expectation_partial_C(Ymat, Yflagmat, logxx.col(j), zero_matrix.col(j), sub_prior_weight, TRUE);
